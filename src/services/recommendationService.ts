...
export async function generateDailyRecommendation(
  ...
  const availableItems = wardrobe.filter(item => !usedItems.includes(item.id));
  const itemsToConsider = availableItems.length >= 2 ? availableItems : wardrobe;

  // Select a random top garment (Kurti, Top, etc.)
  const topCategories = ['Kurti', 'Top', 'Saree', 'Sherwani'];
  ...
  try {
    ...
    // Fallback matching
    const bottomCategories = ['Palazzo', 'Churidar', 'Salwar', 'Lehenga', 'Bottom'];
  ...
  if (!matchingBottom) {
    const otherItems = itemsToConsider.filter(item => item.id !== selectedTop.id);
    if (otherItems.length > 0) {
      matchingBottom = otherItems[Math.floor(Math.random() * otherItems.length)];
    }
  }
...
function generateColorLogic(topDesc: string, bottomDesc: string): string {
  ...
  const colorCombos: Record<string, string> = {
    ...
    'yellow-orange': 'Yellow and orange create a vibrant, energetic combination.',
  };

  const key = `${topColors[0]}-${bottomColors[0]}`;
  const reverseKey = `${bottomColors[0]}-${topColors[0]}`;
  
  return colorCombos[key] || colorCombos[reverseKey] || 
    'The colors complement each other beautifully, creating a harmonious look.';
}

// Generate silhouette coordination logic
function generateSilhouetteLogic(topCategory: string, bottomCategory: string): string {
  const combos: Record<string, string> = {
    'Kurti-Palazzo': 'The flowing Palazzo balances the structured Kurti for elegant comfort.',
    'Kurti-Churidar': 'The fitted Churidar accentuates the Kurti\'s length and detail.',
    'Kurti-Salwar': 'Classic pairing offering traditional elegance and ease of movement.',
    'Top-Lehenga': 'The flared Lehenga creates a dramatic silhouette with the fitted top.',
    'Saree-': 'The draped Saree offers timeless grace and versatility.',
    'Sherwani-Churidar': 'The fitted Churidar complements the structured Sherwani perfectly.',
  };
...
function extractColors(description: string): string[] {
  const colorList = [
    ...
    'white', 'gold', 'silver', 'maroon', 'beige', 'brown', 'grey', 'gray',
    'mustard', 'teal', 'navy', 'coral', 'peach', 'lavender', 'mint',
    'crimson', 'charcoal', 'ivory', 'cream', 'turquoise', 'magenta'
  ];
...
function getOccasionFromOutfit(top: WardrobeItem, bottom: WardrobeItem): string {
  const combined = `${top.analysis.analyzed_garment} ${bottom.analysis.analyzed_garment}`.toLowerCase();
  
  if (combined.includes('silk') || combined.includes('brocade') || combined.includes('zari')) {
...
export function saveRecommendationToHistory(recommendationId: string): void {
  try {
    ...
    localStorage.setItem('fitcheck-recommendation-history', JSON.stringify(trimmed));
  } catch (error) {
